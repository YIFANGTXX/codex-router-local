...
`src/model-failover.mjs` decides when a turn whose provider is limited or
unavailable is rebuilt for a different model, and `buildRoutedRequest` in
...
1. The **same relayed-byte rule as upstream retries**, for the same reason. The
   ...
   path.
2. Only **a final limit or retryable provider failure** qualifies:
   `upstreamFailureKind` returning `out_of_usage`, a 402, any final 429, or a
   final 500/502/503/504 after the provider path's own narrow retries are
   exhausted. Do not add 401 or 403 — a swap would hide the rejected credential
   that is the only thing worth telling the operator. Do not add 404, 400, or
   501, which are deterministic. A recovered 5xx stays visible in usage and the
   always-on failover log, so recovery must never erase the incident.
   A short/unknown 429 and every 5xx can move only the current turn; neither may
   create a later cooldown. Only a long provider-named reset window is durable
   evidence for skipping the user's selected model on a later turn.
