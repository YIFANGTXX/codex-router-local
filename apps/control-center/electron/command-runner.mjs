 ...
 * never pass through a shell. `stdin` carries bounded trusted input that
 * should not appear in a process list, such as credentials or local paths.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    if (stdin === undefined) child.stdin.end();
      ...
      if (typeof stdin !== "string" && !Buffer.isBuffer(stdin)) {
        void abortCommand(new TypeError("Router command input must be text."));
      } else {
...
export function runRouterScriptDetached(
  scriptName,
  args = [],
  {
    sourceRoot = discoverSourceRoot(),
  ...
  return new Promise((resolve, reject) => {
    const child = spawnImpl(runtime.executable, [script, ...args], {
      cwd: sourceRoot,
      detached: true,
      env: childEnvironment,
      shell: false,
    ...
    child.once("error", (error) => {
      if (settled) return;
      settled = true;
      reject(new Error(safeFailure(error?.message || "Detached router script could not start.")));
    });
    child.once("spawn", () => {
      if (settled) return;
      settled = true;
