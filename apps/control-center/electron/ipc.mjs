...
const LOCAL_GGUF_ID = /^[a-z0-9][a-z0-9-]{0,63}$/;
const CONNECTION_ADAPTERS = ["openai-chat", "openai-responses", "openai-completions"];
...
async function testInstalledConnectionInput(input) {
  const modulePath = path.join(discoverSourceRoot(), "src", "generic-providers.mjs");
  const module = await import(pathToFileURL(modulePath).href);
  if (typeof module.testGenericProviderInput !== "function") {
    throw new Error("The installed router does not support connection checks.");
  }
  return module.testGenericProviderInput(input);
}
...
export function registerIpcHandlers({
  ...
  connectionInputTester = testInstalledConnectionInput,
  routingStatusReader = () => ({
    state: "native",
    connected: false,
  ...
  handle("getConnections", async () => runJson(["connections", "list"]));
  handle("getLocalGgufModels", async () => {
    const result = await runRouterScript("local-gguf.mjs", ["list"], { timeoutMs: 20_000 });
    try { return JSON.parse(result.stdout.trim()); }
    catch { throw new Error("Local model registry returned invalid JSON."); }
  });
  handle("discoverLocalGgufModels", async ({ root } = {}) => {
    const directory = stringValue(root, "Search folder");
    if (!path.isAbsolute(directory)) throw new Error("Search folder must be an absolute path.");
    const result = await runRouterScript("local-gguf.mjs", ["discover", directory], { timeoutMs: 60_000 });
    try { return JSON.parse(result.stdout.trim()); }
    catch { throw new Error("Local model discovery returned invalid JSON."); }
  });
  handle("getRoutingStatus", async () => routingStatusReader());
  handleAction("setRoutingConnected", async ({ enabled } = {}) => {
    if (typeof enabled !== "boolean") throw new Error("enabled must be boolean.");
    return routingStatusWriter(enabled);
  }, { requiresCompatibleRouter: false });
  handle("getFallback", async () => runJson(["failover", "status"]));
  handle("getProjectRoute", async ({ project } = {}) => {
    const directory = stringValue(project, "Project folder");
    if (!path.isAbsolute(directory)) throw new Error("Project folder must be an absolute path.");
    const result = await runRouterScript("project-route.mjs", ["status", "--project", directory, "--json"]);
  ...
  handleAction("addConnection", async ({
    id,
    name,
    baseUrl,
    adapter = "openai-chat",
    ...
    const providerId = stringValue(id, "Connection id", PROVIDER_ID);
    const displayName = stringValue(name, "Connection name");
    const endpoint = stringValue(baseUrl, "Base URL");
    oneOf(adapter, CONNECTION_ADAPTERS, "API format");
    if (typeof allowPrivate !== "boolean") throw new Error("allowPrivate must be boolean.");
    if (!Array.isArray(models) || models.length > 50) {
      throw new Error("Add at most 50 model ids.");
    }
    const modelIds = [...new Set(models.map((model) => stringValue(model, "Model id", MODEL_SLUG)))];
    if (modelIds.length !== models.length) throw new Error("Model ids must be unique.");
    if (apiKey !== undefined && (typeof apiKey !== "string" || !apiKey.trim() || apiKey.length > 16 * 1024)) {
      throw new Error("API key is invalid.");
    }
    const window = contextWindow === undefined || contextWindow === null || contextWindow === ""
      ? undefined
      : Number(contextWindow);
    if (window !== undefined && (!Number.isInteger(window) || window < 1 || window > 10_000_000)) {
      throw new Error("Context window must be a positive whole number.");
    }
    return runJson([
      ...
      ...modelIds.flatMap((model) => ["--model", model]),
      ...(window ? ["--context-window", String(window)] : []),
      ...(apiKey ? ["--with-key"] : []),
    ], {
      ...(apiKey ? { stdin: apiKey } : {}),
      timeoutMs: CATALOG_MUTATION_TIMEOUT_MS,
  ...
  handleAction("addLocalGgufModel", async (input = {}) => {
    if (!input || typeof input !== "object" || Array.isArray(input)) throw new Error("Local model input is invalid.");
    const result = await runRouterScript("local-gguf.mjs", ["add"], {
      stdin: JSON.stringify(input),
      timeoutMs: 60_000,
    });
    try { return JSON.parse(result.stdout.trim()); }
    catch { throw new Error("Local model registration returned invalid JSON."); }
  }, { requiresCompatibleRouter: false });
  handleAction("startLocalGgufModel", async ({ id } = {}) => {
    const model = stringValue(id, "Local model", LOCAL_GGUF_ID);
    const result = await runRouterScript("local-gguf.mjs", ["start", model], { timeoutMs: 330_000 });
    try { return JSON.parse(result.stdout.trim()); }
    catch { throw new Error("Local model start returned invalid JSON."); }
  }, { requiresCompatibleRouter: false });
  handleAction("stopLocalGgufModel", async ({ id } = {}) => {
    const model = stringValue(id, "Local model", LOCAL_GGUF_ID);
    const result = await runRouterScript("local-gguf.mjs", ["stop", model], { timeoutMs: 30_000 });
    try { return JSON.parse(result.stdout.trim()); }
    catch { throw new Error("Local model stop returned invalid JSON."); }
  }, { requiresCompatibleRouter: false });
  handleAction("testLocalGgufModel", async ({ id } = {}) => {
    const model = stringValue(id, "Local model", LOCAL_GGUF_ID);
    const result = await runRouterScript("local-gguf.mjs", ["test", model], { timeoutMs: 330_000 });
    try { return JSON.parse(result.stdout.trim()); }
    catch { throw new Error("Local model test returned invalid JSON."); }
  }, { requiresCompatibleRouter: false });
  handleAction("setLocalGgufPublished", async ({ id, published } = {}) => {
    const model = stringValue(id, "Local model", LOCAL_GGUF_ID);
    if (typeof published !== "boolean") throw new Error("published must be boolean.");
    const result = await runRouterScript(
      "local-gguf.mjs",
      [published ? "publish" : "unpublish", model],
      { timeoutMs: CATALOG_MUTATION_TIMEOUT_MS },
    );
    try { return JSON.parse(result.stdout.trim()); }
    catch { throw new Error("Local model publication returned invalid JSON."); }
  });
  handleAction("removeLocalGgufModel", async ({ id } = {}) => {
    const model = stringValue(id, "Local model", LOCAL_GGUF_ID);
    const result = await runRouterScript("local-gguf.mjs", ["remove", model], {
      timeoutMs: CATALOG_MUTATION_TIMEOUT_MS,
    });
    try { return JSON.parse(result.stdout.trim()); }
    catch { throw new Error("Local model removal returned invalid JSON."); }
  });
  handleAction("setConnectionEnabled", async ({ providerId, enabled } = {}) => {
    const id = stringValue(providerId, "Connection id", PROVIDER_ID);
    if (typeof enabled !== "boolean") throw new Error("enabled must be boolean.");
    return runJson(["connections", enabled ? "enable" : "disable", id], {
      timeoutMs: CATALOG_MUTATION_TIMEOUT_MS,
    });
  });
  handleAction("updateConnection", async ({
    providerId,
    name,
    baseUrl,
    adapter = "openai-chat",
  ...
  handleAction("setConnectionCredential", async ({ providerId, credential } = {}) => {
    const id = stringValue(providerId, "Connection id", PROVIDER_ID);
    if (typeof credential !== "string" || !credential.trim() || credential.length > 16 * 1024) {
      throw new Error("API key is invalid.");
    }
    return runJson(["connections", "key-set", id], {
      stdin: credential,
      timeoutMs: CATALOG_MUTATION_TIMEOUT_MS,
    });
  });
  handleAction("clearConnectionCredential", async ({ providerId } = {}) => (
    runJson(["connections", "key-clear", stringValue(providerId, "Connection id", PROVIDER_ID)], {
      timeoutMs: CATALOG_MUTATION_TIMEOUT_MS,
    })
  ));
  handleAction("addConnectionModel", async ({ providerId, modelId, contextWindow } = {}) => {
    const id = stringValue(providerId, "Connection id", PROVIDER_ID);
    ...
    return runJson([
      "connections", "model-add", id, "--model", model,
      ...(window ? ["--context-window", String(window)] : []),
    ], { timeoutMs: CATALOG_MUTATION_TIMEOUT_MS });
  });
  handleAction("removeConnectionModel", async ({ providerId, modelId } = {}) => (
    runJson([
      "connections", "model-remove",
      stringValue(providerId, "Connection id", PROVIDER_ID),
      "--model", stringValue(modelId, "Model id", MODEL_SLUG),
    ], { timeoutMs: CATALOG_MUTATION_TIMEOUT_MS })
  ));
  handleAction("testConnection", async ({ providerId } = {}) => (
    runJson(["connections", "test", stringValue(providerId, "Connection id", PROVIDER_ID)], {
      timeoutMs: 30_000,
    })
  ));
  handle("discoverConnectionModels", async ({ providerId } = {}) => (
    runJson(["connections", "discover-models", stringValue(providerId, "Connection id", PROVIDER_ID)], {
      timeoutMs: 45_000,
    })
  ));
  handleAction("testConnectionInput", async ({
    baseUrl,
    adapter = "openai-chat",
    allowPrivate = false,
    apiKey,
  ...
  handleAction("removeConnection", async ({ providerId } = {}) => (
    runJson(["connections", "remove", stringValue(providerId, "Connection id", PROVIDER_ID)], {
      timeoutMs: CATALOG_MUTATION_TIMEOUT_MS,
    })
  ));
  handleAction("setFallback", async ({ enabled, chain } = {}) => {
    if (typeof enabled !== "boolean") throw new Error("enabled must be boolean.");
    if (!Array.isArray(chain) || chain.length > 12) throw new Error("Fallback chain is invalid.");
    const models = chain.map((model) => stringValue(model, "Fallback model", MODEL_SLUG));
    if (new Set(models).size !== models.length) throw new Error("Fallback models must be unique.");
    await runJson(["failover", "chain", ...models]);
    return runJson(["failover", enabled ? "on" : "off"]);
  });
  handleAction("setProjectRoute", async ({ project, model } = {}) => {
    const directory = stringValue(project, "Project folder");
    if (!path.isAbsolute(directory)) throw new Error("Project folder must be an absolute path.");
    const slug = stringValue(model, "Model", MODEL_SLUG);
    const result = await runRouterScript(
      "project-route.mjs",
      ["set", slug, "--project", directory, "--json"],
    );
    return JSON.parse(result.stdout.trim());
  });
  handleAction("clearProjectRoute", async ({ project } = {}) => {
    const directory = stringValue(project, "Project folder");
    if (!path.isAbsolute(directory)) throw new Error("Project folder must be an absolute path.");
    const result = await runRouterScript(
      "project-route.mjs",
      ["clear", "--project", directory, "--json"],
    );
    return JSON.parse(result.stdout.trim());
  });
