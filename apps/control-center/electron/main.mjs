...
import {
  runControl,
  runRouterScript,
  runRouterScriptDetached,
} from "./command-runner.mjs";
import { createRoutingLifecycle } from "./routing-lifecycle.mjs";
import {
...
let deferredQuit;
let lifecycleHeartbeat;
let routingHealthMonitor;
let applicationReady = false;
let windowVisible = false;
let windowContentReady = false;
let showWhenContentReady = false;
const lifecycleFile = lifecycleStatePath();
const routingLifecycle = createRoutingLifecycle({ runControl, runScript: runRouterScript });
...
const connectInvocation = process.argv.includes("--connect");
...
function createWindow() {
  ...
  createdWindow.on("hide", () => {
    // Minimizing is not disconnecting. Users routinely minimize the shell to
    // work in Codex; keep its connection boundary active in that case.
    if (createdWindow.isMinimized()) return;
    windowVisible = false;
    publishLifecycleState();
    void routingLifecycle.request(false);
...
if (primaryInstance && !quitForUpdateInvocation) {
  ...
  publishLifecycleState();
  lifecycleHeartbeat = setInterval(publishLifecycleState, 2_000);
  lifecycleHeartbeat.unref?.();
  void routingLifecycle.request(connectInvocation);
  routingHealthMonitor = setInterval(() => {
    void routingLifecycle.ensureHealthy();
  }, 4_000);
  routingHealthMonitor.unref?.();
  void runRouterScriptDetached("control-center-watchdog.mjs", [String(process.pid)])
    .catch(() => { /* normal shutdown still restores native mode */ });
  app.whenReady().then(() => {
    ...
    mutationLifecycle = registerIpcHandlers({
      ...
      routingStatusReader: routingLifecycle.snapshot,
      routingStatusWriter: (enabled) => routingLifecycle.request(enabled),
    });
...
if (primaryInstance) app.on("second-instance", (_event, commandLine) => {
  if (commandLine.includes("--quit-for-update")) {
    ...
    if (commandLine.includes("--connect")) void routingLifecycle.request(true);
  }
});

if (primaryInstance) app.on("before-quit", (event) => {
  void routingLifecycle.request(false);
  if (!mutationLifecycle.hasActiveMutations() && !routingLifecycle.hasActiveMutation()) return;
  event.preventDefault();
  if (!deferredQuit) {
    deferredQuit = Promise.all([
      mutationLifecycle.whenMutationsIdle(),
      routingLifecycle.whenIdle(),
    ]).then(() => {
...
if (primaryInstance) app.on("will-quit", () => {
  ...
  clearInterval(routingHealthMonitor);
  applicationReady = false;
  windowVisible = false;
  publishLifecycleState();
});
