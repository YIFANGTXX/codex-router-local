...
export interface RoutingStatus {
  state: "native" | "connecting" | "connected" | "disconnecting" | "error" | string;
  connected: boolean;
...
export type ConnectionAdapter = "openai-chat" | "openai-responses" | "openai-completions";
...
export interface ConnectionSnapshot {
  version: number;
  security: {
    canonicalHistory: "codex-local" | string;
    keys: "protected-local-files" | string;
    externalDefaults: number;
    note: string;
  };
...
export type UpdateConnectionInput = Pick<AddConnectionInput, "name" | "baseUrl" | "adapter" | "allowPrivate">;
...
export interface ProjectRouteSnapshot {
  project: string;
  path: string;
  managed: boolean;
  model?: string;
...
export interface LocalGgufCandidate {
  id: string;
  displayName: string;
  modelId: string;
  modelPath: string;
  ...
  runtimeKind?: "llama-server" | "python-llama-cpp";
  inputModalities?: Array<"text" | "image">;
  thinkingEnabled?: boolean;
  reasoningEffort?: "low" | "medium" | "xhigh";
  reasoningLevels?: Array<"low" | "medium" | "xhigh">;
  sizeBytes: number;
  gpuLayers?: number;
  contextWindow?: number;
  maxOutputTokens?: number;
...
export interface LocalGgufModel extends Required<Pick<LocalGgufCandidate,
  "id" | "displayName" | "modelId" | "modelPath" | "pythonPath" | "dependenciesPath" |
  "mmprojPath" | "serverPath" | "runtimeKind" | "inputModalities" | "thinkingEnabled" | "reasoningEffort" | "reasoningLevels" |
  "sizeBytes" | "gpuLayers" | "contextWindow" | "maxOutputTokens" | "temperature" | "topP" |
  "topK" | "minP" | "repeatPenalty" | "frequencyPenalty" | "presencePenalty" | "stopSequences" | "port" |
  "tuningPreset"
>> {
  createdAt: string;
  available: boolean;
  ...
  published: boolean;
  startup?: { state: "starting" | "succeeded" | "timeout" | "failed"; startedAt: string; finishedAt?: string } | null;
}

export interface LocalGgufSnapshot {
  version: number;
  security: {
    binding: "127.0.0.1" | string;
    canonicalHistory: "codex-local" | string;
    uploads: boolean;
    note: string;
  };
  active: { id: string; status: "loading" | "running" | "error"; startedAt: string } | null;
  models: LocalGgufModel[];
}

export interface LocalGgufDiscovery {
  root: string;
  candidates: LocalGgufCandidate[];
  truncated: boolean;
}

export interface LocalGgufTestResult {
  ok: boolean;
  model: string;
  text: string;
  elapsedMs: number;
...
export interface RouterControlApi {
  ...
  setRoutingConnected(enabled: boolean): Promise<RoutingStatus>;
  getProviders(): Promise<ProviderSetupSnapshot>;
  getConnections(): Promise<ConnectionSnapshot>;
  getLocalGgufModels(): Promise<LocalGgufSnapshot>;
  discoverLocalGgufModels(root: string): Promise<LocalGgufDiscovery>;
  addLocalGgufModel(input: LocalGgufCandidate): Promise<LocalGgufSnapshot>;
  startLocalGgufModel(id: string): Promise<LocalGgufSnapshot>;
  stopLocalGgufModel(id: string): Promise<LocalGgufSnapshot>;
  testLocalGgufModel(id: string): Promise<LocalGgufTestResult>;
  setLocalGgufPublished(id: string, published: boolean): Promise<LocalGgufSnapshot>;
  removeLocalGgufModel(id: string): Promise<LocalGgufSnapshot>;
  addConnection(input: AddConnectionInput): Promise<ConnectionSnapshot>;
  updateConnection(provider: string, input: UpdateConnectionInput): Promise<ConnectionSnapshot>;
  setConnectionEnabled(provider: string, enabled: boolean): Promise<ConnectionSnapshot>;
  setConnectionCredential(provider: string, credential: string): Promise<ConnectionSnapshot>;
  clearConnectionCredential(provider: string): Promise<ConnectionSnapshot>;
  addConnectionModel(provider: string, model: string, contextWindow?: number): Promise<ConnectionSnapshot>;
  removeConnectionModel(provider: string, model: string): Promise<ConnectionSnapshot>;
  testConnection(provider: string): Promise<ConnectionTestResult>;
  testConnectionInput(input: Pick<AddConnectionInput, "baseUrl" | "adapter" | "allowPrivate" | "apiKey">): Promise<ConnectionTestResult>;
  discoverConnectionModels(provider: string): Promise<ConnectionModelCatalog>;
  removeConnection(provider: string): Promise<ConnectionSnapshot>;
  getFallback(): Promise<FallbackSnapshot>;
  setFallback(enabled: boolean, chain: string[]): Promise<FallbackSnapshot>;
  getProjectRoute(project: string): Promise<ProjectRouteSnapshot>;
  setProjectRoute(project: string, model: string): Promise<ProjectRouteSnapshot>;
  clearProjectRoute(project: string): Promise<ProjectRouteSnapshot>;
