...
test("classifyRoutedFailure swaps every final rate limit but cools only a long one", () => {
  ...
  assert.deepEqual(short, { swap: true, reason: "rate_limited" });
...
test("classifyRoutedFailure swaps after retryable provider failures are exhausted", () => {
  for (const status of [500, 502, 503, 504]) {
    assert.deepEqual(classifyRoutedFailure({ status, now: NOW }), {
      swap: true,
      reason: "provider_unavailable",
    });
  }
  assert.equal(classifyRoutedFailure({ status: 501, now: NOW }).swap, false);
});
...
test("classifyRoutedFailure leaves every other failure exactly as it was", () => {
  const cases = [
    ...
    { status: 408, bodyText: quotaBody("Request timeout") },
    { status: 409, bodyText: quotaBody("Conflict") },
  ];
...
test("failover is off until the operator explicitly enables it", () => {
