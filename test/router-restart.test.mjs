...
test("restart is skipped when no background service is installed", () => {
  ...
  assert.equal(restartRouterServiceIfInstalled({ spawn, platform: "linux" }), false);
  assert.deepEqual(calls, ["status"]);
});

test("Windows falls back to the identity-checked local launcher when Task Scheduler is unavailable", () => {
  const calls = [];
  const spawn = (command, args) => {
    calls.push(path.basename(args.at(-1)));
    return calls.length === 1
      ? NOT_INSTALLED_STATUS
      : { status: 0, error: undefined, stdout: "", stderr: "" };
  };
  assert.equal(restartRouterServiceIfInstalled({ spawn, platform: "win32" }), true);
  assert.deepEqual(calls, ["status", "router-restart-fallback.mjs"]);
});
