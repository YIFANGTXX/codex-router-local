...
test("pipeResponse keeps a silent local SSE prefill alive without fabricating output", async () => {
  const server = http.createServer(async (_request, response) => {
    ...
    await pipeResponse(upstream, response, new Set(), [], { sseHeartbeatMs: 5 });
  });
  ...
  assert.match(body, /^event: codex\.router\.keepalive/m);
  assert.match(body, /data: model-output/);
  assert.ok(
    body.match(/event: codex\.router\.keepalive/g).length >= 2,
    "the silent prefill did not receive periodic SSE heartbeats",
  );
});

test("local heartbeat never splits fragmented model JSON and preserves transforms", async () => {
  ...
  const server = http.createServer(async (_request, response) => {
    const upstream = new Response(new ReadableStream({
      start(controller) {
        ...
        setTimeout(() => controller.enqueue(Buffer.from(suffix)), 35);
        setTimeout(() => controller.close(), 65);
      },
    }), { headers: { "content-type": "text/event-stream" } });
    await pipeResponse(upstream, response, new Set(), transforms, { sseHeartbeatMs: 5 });
  });
  const port = await listen(server);
  try {
    const body = await fetch(`http://127.0.0.1:${port}/`).then(r => r.text());
    const data = body.split("\n\n").filter(Boolean).map(frame => JSON.parse(frame.split("\ndata: ")[1]));
    assert.deepEqual(data.filter(frame => frame.type !== "codex.router.keepalive"), [
      { type: "response.output_text.delta", delta: "千问" },
    ]);
    assert.equal(transforms.length, 0, "the caller's transform list was mutated");
    assert.ok(data.filter(frame => frame.type === "codex.router.keepalive").length > 1);
  } finally { await close(server); }
});

test("heartbeat is opt-in and never added to native or JSON responses", async () => {
  for (const [contentType, options] of [["text/event-stream", {}], ["application/json", { sseHeartbeatMs: 1 }]]) {
    const original = '{"untouched":true}';
    const server = http.createServer(async (_request, response) => {
      await pipeResponse(new Response(original, { headers: { "content-type": contentType } }), response, new Set(), [], options);
    });
    const port = await listen(server);
    try { assert.equal(await fetch(`http://127.0.0.1:${port}/`).then(r => r.text()), original); }
    finally { await close(server); }
  }
});
