...
test("a final provider 503 is served by the fallback without leaking failed bytes", async () => {
  const seen = [];
  const gw = await gateway(async (request, response) => {
    const body = await bodyJson(request);
    seen.push(body);
    ...
    response.writeHead(200, { "Content-Type": "text/event-stream" });
    response.end(contentSse("503-fallback"));
  });
  const routerPort = await openPort();
  const child = run(routerEnv(gw.port, routerPort));
  try {
    ...
    assert.equal(result.status, 200);
    assert.match(result.body, /answered-by-503-fallback/u);
    assert.doesNotMatch(result.body, /temporarily unavailable/u);
    assert.match(child.testErrors(), /reason=provider_unavailable/u);
    const events = await waitForUsageEvents(child.stateDir, 2, child);
    assert.equal(events.find((event) => event.model === PRIMARY.slug).status, 503);
