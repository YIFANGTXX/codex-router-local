...
test("closing the hot-switch window ends its Windows and Linux routing session", () => {
  ...
  assert.equal(shouldQuitOnLastWindowClosed({ platform: "win32", nativeTrayOwnedByHost: false, trayAvailable: true }), true);
  assert.equal(shouldQuitOnLastWindowClosed({ platform: "win32", nativeTrayOwnedByHost: false, trayAvailable: false }), true);
  assert.equal(shouldQuitOnLastWindowClosed({ platform: "linux", nativeTrayOwnedByHost: false, trayAvailable: true }), true);
...
test("electron boundary does not enable node integration or shell argv", async () => {
  ...
  assert.match(main, /function revealWindow\(\)[\s\S]{0,420}showDockForVisibleWindow\(\)[\s\S]{0,120}mainWindow\.show\(\)/);
    assert.match(main, /createdWindow\.on\("hide"[\s\S]*?hideDockForHiddenWindow\(\)/);
  ...
  assert.match(main, /else \{\s*openRequests\.requestOpen\(\);\s*if \(commandLine\.includes\("--connect"\)\) void routingLifecycle\.request\(true\)/);
  ...
  assert.match(main, /queryLifecycleState\(lifecycleFile\)/);
    assert.match(main, /createdWindow\.on\("hide"[\s\S]*?windowVisible = false/);
  ...
  assert.match(builder, /nsis:[\s\S]*runAfterFinish:\s*false/);
...
test("preload constructs exact positional IPC payloads", async () => {
  ...
  const cases = [
    ["discoverLocalGgufModels", ["C:\\Models"], { root: "C:\\Models" }],
    ["setRoutingConnected", [true], { enabled: true }],
    ["addLocalGgufModel", [{ id: "qwen-local" }], { id: "qwen-local" }],
    ["startLocalGgufModel", ["qwen-local"], { id: "qwen-local" }],
    ["stopLocalGgufModel", ["qwen-local"], { id: "qwen-local" }],
    ["testLocalGgufModel", ["qwen-local"], { id: "qwen-local" }],
    ["setLocalGgufPublished", ["qwen-local", true], { id: "qwen-local", published: true }],
    ["removeLocalGgufModel", ["qwen-local"], { id: "qwen-local" }],
    ...
    ["updateConnection", ["provider", { name: "Provider", baseUrl: "https://api.example.test/v1", adapter: "openai-responses", allowPrivate: false }], { providerId: "provider", name: "Provider", baseUrl: "https://api.example.test/v1", adapter: "openai-responses", allowPrivate: false }],
    ...
    ["testConnectionInput", [{ baseUrl: "https://api.example.test/v1", adapter: "openai-responses", allowPrivate: false }], { baseUrl: "https://api.example.test/v1", adapter: "openai-responses", allowPrivate: false }],
    ["discoverConnectionModels", ["provider"], { providerId: "provider" }],
