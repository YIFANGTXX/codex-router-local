...
function curatedQwen38EffortModel({ requestProfile = "qwen38-community" } = {}) {
  const dir = mkdtempSync(path.join(os.tmpdir(), `routing-${requestProfile}-models-`));
  const file = path.join(dir, "user-models.json");
  writeFileSync(
    file,
    JSON.stringify({
...
test("local Qwen compacts tool prose without changing history or executable tool contracts", async () => {
  const upstreamRequests = [];
  const upstream = await mockServer(async (request, response) => {
    upstreamRequests.push({ headers: request.headers, body: await bodyJson(request) });
    json(response, 200, { choices: [] });
  });
  const curated = curatedQwen38EffortModel({ requestProfile: "qwen38-local" });
  const forwarderPort = await openPort();
  const forwarder = run("api-forwarder.mjs", {
    CODEX_ROUTER_API_PORT: String(forwarderPort),
    MODEL_ROUTER_USER_MODELS: curated.file,
  ...
  const messages = [
    { role: "system", content: "canonical system marker" },
    { role: "user", content: "only this task's canonical history marker" },
  ];
  const longToolDescription = `Choose this tool for shell work. ${"selection detail ".repeat(80)}Final safety rule.`;
  const longParameterDescription = `Absolute command input. ${"schema detail ".repeat(40)}Keep the final constraint.`;
  const tools = [{
    type: "function",
    function: {
      name: "shell",
  ...
  try {
    ...
    assert.equal(response.status, 200);
    const sent = upstreamRequests.at(-1).body;
    assert.equal(sent.cache_prompt, true);
    assert.deepEqual(sent.messages, messages, "the provider projection changed canonical history");
    assert.equal(sent.tools.length, 1, "a Codex tool was removed");
    assert.equal(sent.tools[0].function.name, "shell");
    assert.equal(sent.tools[0].function.strict, true);
    assert.equal(sent.tool_choice, "required");
    assert.deepEqual(sent.tools[0].function.parameters.required, ["command"]);
    assert.equal(sent.tools[0].function.parameters.additionalProperties, false);
    assert.deepEqual(
      sent.tools[0].function.parameters.properties.command.enum,
      ["status", "inspect"],
    );
    assert.ok(sent.tools[0].function.description.length <= 384);
    assert.match(sent.tools[0].function.description, /^Choose this tool/);
    assert.match(sent.tools[0].function.description, /Final safety rule\.$/);
    const parameterDescription =
      sent.tools[0].function.parameters.properties.command.description;
    assert.ok(parameterDescription.length <= 160);
    assert.match(parameterDescription, /^Absolute command input/);
    assert.match(parameterDescription, /Keep the final constraint\.$/);
  } finally {
    await stopChild(forwarder);
    await closeServer(upstream.server);
    rmSync(curated.dir, { recursive: true, force: true });
...
test("routed compaction records usage and logs on success and on failure", async () => {
  ...
  const gateway = await mockServer(async (request, response) => {
    await bodyJson(request);
    if (failing) {
      // 500/502/503/504 are now recoverable before any byte reaches Codex.
      // Use the deliberately non-retryable 501 here so this test keeps
      // exercising the terminal-failure telemetry path rather than failover.
      json(response, 501, { error: { message: "provider refused the compaction" } });
      return;
    }
    json(response, 200, {
      id: "resp-summary",
  ...
  try {
    ...
    await waitForStderr(
      router,
      /\[codex-router\] model=deepseek\/deepseek-v4-pro provider=deepseek status=501/,
    );
  } finally {
    await stopChild(router);
    await closeServer(gateway.server);
