...
test("public installation docs describe user-owned connections without requesting secrets in chat", () => {
  for (const file of ["README.md", path.join("docs", "INSTALL.md")]) {
    const contents = readFileSync(path.join(root, file), "utf8");
    assert.match(contents, /API Key/);
    assert.match(contents, /自己|你填写|使用者/);
    assert.doesNotMatch(contents, /请(?:把|将) API Key 粘贴/);
  }
});
