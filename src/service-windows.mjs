...
import { startManagedRouterFallback } from "./router-restart-fallback.mjs";
...
if (command === "render") {
  ...
  command === "start"
  && HOST_MANAGED
  && !skipServiceManagerCall({ hostManaged: HOST_MANAGED })
  && !taskExists()
) {
  // The launcher may exist even when Task Scheduler registration was refused.
  // The Control Center's manual Connect button still needs a safe way to bring
  // up this user's own router, without installing a task or changing a proxy.
  await startManagedRouterFallback({ platform: effectivePlatform });
  process.stdout.write(`${JSON.stringify({ state: "running", fallback: "local-launcher" })}\n`);
} else {
