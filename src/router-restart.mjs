...
const FALLBACK_RESTART_SCRIPT = path.join(SOURCE_ROOT, "src", "router-restart-fallback.mjs");
...
export function restartRouterServiceIfInstalled({
  spawn = spawnSync,
  env = process.env,
  ...
  if (!status.installed) {
    ...
    const fallback = spawn(process.execPath, [FALLBACK_RESTART_SCRIPT], {
      cwd: SOURCE_ROOT,
      env,
      encoding: "utf8",
    ...
    if (fallback?.status === 3) return false;
    if (fallback?.status !== 0) {
      throw new Error(
        String(fallback?.stderr || "").trim() ||
        "The locally managed router could not be restarted; the new model route is not live yet.",
      );
    }
    return true;
  }
