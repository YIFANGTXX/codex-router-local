...
import { apiForwarderHealthPayload } from "./api-forwarder-health.mjs";
import { ensureLocalModelForRequest, LocalRuntimeError } from "./local-gguf-demand.mjs";
...
// The local Qwen route logs sizes only -- never prompt, tool, or credential
// contents -- so a slow cold start can be diagnosed without turning the router
// log into another copy of the canonical Codex history.
const LOCAL_QWEN_TOOL_DESCRIPTION_CHARS = 384;
const LOCAL_QWEN_SCHEMA_DESCRIPTION_CHARS = 160;
...
function jsonBytes(value) {
  ...
  return typeof encoded === "string" ? Buffer.byteLength(encoded, "utf8") : 0;
}
...
function compactDescription(value, limit) {
  ...
  return `${normalized.slice(0, head)}${separator}${normalized.slice(-(available - head))}`;
}

// Descriptions teach a model how to choose a tool, but they are not part of a
// tool's executable contract. Preserve every schema key, property, enum,
// required field, and strictness flag while bounding only prose. Head and tail
// are both retained because provider caveats often live at the end.
function compactSchemaDescriptions(value) {
  if (Array.isArray(value)) return value.map((entry) => compactSchemaDescriptions(entry));
  if (!value || typeof value !== "object") return value;
  return Object.fromEntries(
    Object.entries(value).map(([key, entry]) => [
      key,
      key === "description"
        ? compactDescription(entry, LOCAL_QWEN_SCHEMA_DESCRIPTION_CHARS)
        : compactSchemaDescriptions(entry),
    ]),
  );
}

function compactLocalQwenTools(tools) {
  if (!Array.isArray(tools)) return tools;
  return tools.map((tool) => {
    const compacted = compactSchemaDescriptions(tool);
    if (!tool || typeof tool !== "object" || !compacted || typeof compacted !== "object") {
      return compacted;
    }
    if (typeof tool.description === "string") {
      compacted.description = compactDescription(
        tool.description,
        LOCAL_QWEN_TOOL_DESCRIPTION_CHARS,
      );
    }
    if (
      tool.function &&
      ...
      compacted.function &&
      typeof compacted.function === "object" &&
      typeof tool.function.description === "string"
    ) {
      compacted.function.description = compactDescription(
...
function logLocalQwenContext(model, before, payload) {
  const afterTools = jsonBytes(payload.tools);
  const afterRequest = jsonBytes(payload);
  console.error(
    `[api-forwarder] local-qwen-context model=${model.gatewayModel} ` +
      `messages=${Array.isArray(payload.messages) ? payload.messages.length : 0} ` +
      `message_bytes=${jsonBytes(payload.messages)} ` +
      `tools=${Array.isArray(payload.tools) ? payload.tools.length : 0} ` +
      `request_bytes=${before.request}->${afterRequest} ` +
      `tool_bytes=${before.tools}->${afterTools} ` +
      `saved_bytes=${Math.max(0, before.request - afterRequest)} history=unchanged`,
  );
}
...
function normalizeBody(buffer, contentType, route) {
  ...
  if (model.requestProfile === "clinepass") {
  ...
  } else if (["qwen38-community", "qwen38-local"].includes(model.requestProfile)) {
    ...
    if (model.requestProfile === "qwen38-local") {
      const before = {
        request: jsonBytes(payload),
        tools: jsonBytes(payload.tools),
      };
      // llama-server's slot/checkpoint cache lives in local RAM. Opt in on the
      // request as well as the server so a new Codex task can reuse the common
      // system/tool prefix without changing or persisting Codex's history.
      payload.cache_prompt = true;
      if (Array.isArray(payload.tools) && payload.tools.length > 0) {
        payload.tools = compactLocalQwenTools(payload.tools);
      }
      logLocalQwenContext(model, before, payload);
    }
...
function upstreamHeaders(requestHeaders, body, apiKey, provider, extraHeaders = {}, endpoint = provider) {
  ...
  if (provider.generic === true) {
    // The generic-provider boundary injects only the credential reference that
    // belongs to this user-created endpoint. Never forward the gateway's
    // internal bearer token or synthesize an Authorization header here.
...
function healthPayload() {
  return apiForwarderHealthPayload({
    providers: PROVIDERS,
    readSelection: readProviderSelection,
    credentialStatus,
    genericCredentialStatus: genericProviderCredentialStatus,
  });
}
...
async function handleRequest(request, response) {
  ...
  const credential = normalized.provider.generic === true
    ? genericProviderCredentialStatus(normalized.provider.id)
    : resolveProviderCredential(normalized.endpoint);
  if (!credential || credential.configured === false) {
    const setup = normalized.provider.generic === true
      ? credential?.setup
  ...
  if (normalized.provider.generic === true) {
    await ensureLocalModelForRequest(normalized.model, normalized.provider, controller.signal);
    const genericHeaders = upstreamHeaders(
      request.headers,
      normalized.body,
      undefined,
      normalized.provider,
  ...
  handleRequest(request, response).catch((error) => {
    ...
    if (!response.headersSent) {
      writeJson(response, status, {
        error: {
          type: error instanceof LocalRuntimeError ? "local_runtime_unavailable" : "provider_api_proxy_error",
          message: error instanceof LocalRuntimeError ? error.message : "The API-provider forwarder could not complete the request.",
        },
      });
