...
import { genericProviderCredentialStatus } from "./generic-providers.mjs";
...
export function configuredProviderIds() {
  ...
  for (const provider of PROVIDERS.values()) {
    ...
    } else if (provider.generic === true) {
      if (genericProviderCredentialStatus(provider.id).configured) configured.push(provider.id);
