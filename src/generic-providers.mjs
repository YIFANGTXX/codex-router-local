...
export function genericProviderCredentialStatus(providerOrId) {
  const provider = typeof providerOrId === "string"
    ? getGenericProvider(providerOrId)
    : validateProvider(providerOrId, { existingId: providerOrId.id });
  if (!provider.credentialRef) {
    return {
      configured: true,
      source: provider.allowPrivate
        ? "user-approved local endpoint (no key)"
        : "user-approved endpoint (no key)",
      persistent: true,
    };
  }
  return credentialSecret(provider)
    ? { configured: true, source: "protected file", persistent: true }
    : {
        configured: false,
        setup: `Save the API key for user-created provider ${provider.id}.`,
      };
}
...
export function genericForwardTimeoutMs(model, provider) {
  if (model?.requestProfile !== "qwen38-local" || provider?.allowPrivate !== true) {
    return 120_000;
  }
  try {
    const host = new URL(provider.baseUrl).hostname.toLowerCase();
    return ["127.0.0.1", "localhost", "[::1]"].includes(host) ? null : 120_000;
  } catch {
    return 120_000;
  }
}
...
function requestSignal(signal, timeoutMs) {
  if (timeoutMs === null) {
    if (!signal) throw new Error("A caller signal is required for a provider request without a timeout.");
    return signal;
  }
...
async function requestProviderDescriptor(
  provider,
  requestPath,
  { secret, fetchImpl = undiciFetch, lookup = lookupHost, timeoutMs = 10_000, ...init } = {},
) {
...
export async function requestGenericProvider(
  ...
  return requestProviderDescriptor(provider, requestPath, {
    ...init,
    fetchImpl,
    lookup,
    timeoutMs,
...
async function testProviderDescriptor(
  provider,
  { secret, fetchImpl = undiciFetch, lookup = lookupHost, timeoutMs = 10_000 } = {},
) {
  const startedAt = Date.now();
  const { response, endpoint, dispatcher } = await requestProviderDescriptor(provider, "/models", {
    secret,
    fetchImpl,
    lookup,
    timeoutMs,
    ...
    return {
      ...
      ...(response.ok
        ...
        : [401, 403].includes(response.status)
          ? { message: "Provider endpoint is reachable but rejected the API key." }
          : { message: "Provider endpoint is reachable but returned an error." }),
    };
  } finally {
    await dispatcher?.close().catch(() => undefined);
  }
...
export async function testGenericProvider(id, options = {}) {
  const provider = getGenericProvider(id);
  if (!provider.enabled) throw new Error(`Generic provider ${provider.id} is disabled.`);
  const secret = credentialSecret(provider);
  if (provider.credentialRef && !secret) {
  ...
  return testProviderDescriptor(provider, { ...options, secret });
}

// The add-connection form can probe an address without first persisting a
// provider or credential. The descriptor still goes through the same URL,
// private-network, DNS, redirect, timeout, and response-size boundaries as a
// saved connection.
export async function testGenericProviderInput({
  baseUrl,
  adapter = "openai-chat",
  allowPrivate = false,
  apiKey,
} = {}, options = {}) {
  if (apiKey !== undefined && (typeof apiKey !== "string" || apiKey.length > 16 * 1024)) {
    throw new Error("API key is invalid.");
  }
  const provider = validateProvider({
    id: "connection-probe",
    displayName: "Connection probe",
    baseUrl,
    adapter,
    allowPrivate,
    headers: {},
  ...
  return testProviderDescriptor(provider, {
    ...options,
    secret: text(apiKey) || undefined,
  });
}
