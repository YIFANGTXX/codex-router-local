...
// Redirected Windows PowerShell output otherwise uses the legacy code page,
// corrupting registered model paths containing Chinese characters.
const WINDOWS_UTF8_OUTPUT = "[Console]::OutputEncoding = [System.Text.UTF8Encoding]::new($false); ";
  ...
  try {
    if (platform === "win32") {
      ...
      for (const script of scripts) {
        const result = spawn(
          "powershell.exe",
          ["-NoLogo", "-NoProfile", "-NonInteractive", "-Command", WINDOWS_UTF8_OUTPUT + script],
          {
            encoding: "utf8",
            windowsHide: true,
            timeout: WINDOWS_PROCESS_PROBE_TIMEOUT_MS,
