...
// fact the operator needs. Upstream retry absorbs transient 5xx responses
// before this classifier sees them. A final 500/502/503/504 is safe to move
// only while no byte has reached Codex; the failed attempt remains visible in
// usage and router logs, so recovering the turn does not erase the incident.
export function classifyRoutedFailure({ status, bodyText, retryAfterSeconds, now } = {}) {
  ...
  if (code === 429) {
    return {
      swap: true,
      reason: "rate_limited",
      // Short or unknown rate limits are eligible for this turn, but they do
      // not cool the provider for later turns. Only a provider's own long
      // reset window is durable routing evidence.
      ...(Number.isFinite(retryAfter) && retryAfter > MIN_RATE_LIMIT_COOLDOWN_SECONDS
        ? { until: cappedUntil(at, retryAfter * 1_000) }
        : {}),
    };
  }
...
function defaultSettings() {
  // A private connection shell must never choose a second destination before
  // the operator explicitly enables failover. This remains off even after the
  // first user-created connection is added; the Control Center records the
  // opt-in together with the user's ordered chain.
