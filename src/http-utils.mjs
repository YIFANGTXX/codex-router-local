...
import { LOCAL_SSE_HEARTBEAT, SseFrameBoundary } from "./sse-heartbeat.mjs";
...
export async function pipeResponse(
  ...
  denylist,
  transform,
  { leaveOpen = false, sseHeartbeatMs = 0 } = {},
) {
  ...
  let heartbeat;
  const eventStream = String(response.getHeader("content-type") || "")
    .toLowerCase()
    .includes("text/event-stream");
  if (eventStream && Number.isFinite(sseHeartbeatMs) && sseHeartbeatMs > 0) {
    // Data-bearing extension events reach Codex's parser; comments alone do
    // not reset its SSE idle deadline. Never fabricate assistant output or
    // inject a frame into the middle of a fragmented model event.
    const boundary = new SseFrameBoundary();
    transforms.push(boundary);
    response.flushHeaders();
    response.write(LOCAL_SSE_HEARTBEAT);
    heartbeat = setInterval(() => {
      if (boundary.atBoundary && !response.destroyed && !response.writableEnded && !response.writableNeedDrain) {
        response.write(LOCAL_SSE_HEARTBEAT);
      }
    }, Math.floor(sseHeartbeatMs));
    heartbeat.unref?.();
  }
