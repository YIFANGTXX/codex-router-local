...
import { runtimeGenericProviderDescriptors } from "./generic-provider-registry.mjs";
...
function loadRegistry() {
  ...
  // User-created endpoints are local state, not checked-in provider presets.
  // Merge only endpoints the operator explicitly enabled, and refuse an id
  // collision instead of letting mutable state replace a reviewed registry
  // provider. Their secrets are still resolved at the request boundary; the
  // descriptor added here contains only routing metadata and redacted headers.
  for (const genericProvider of runtimeGenericProviderDescriptors()) {
    if (!genericProvider.enabled) continue;
    if (providers.has(genericProvider.id)) {
      fail(`generic provider id ${genericProvider.id} collides with a checked-in provider`);
    }
    providers.set(
      genericProvider.id,
      Object.freeze(genericProvider),
    );
  }
