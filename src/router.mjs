...
const EMPTY_COMPLETION_PRELUDE_BYTES =
  ...
  configuredEmptyCompletionPreludeBytes >= 0
    ...
    : 1024 * 1024;
const configuredLocalSseHeartbeatMs = Number(
  process.env.CODEX_ROUTER_LOCAL_SSE_HEARTBEAT_MS || 15_000,
);
const LOCAL_SSE_HEARTBEAT_MS =
  Number.isFinite(configuredLocalSseHeartbeatMs) && configuredLocalSseHeartbeatMs > 0
    ? Math.floor(configuredLocalSseHeartbeatMs)
    : 15_000;
...
// Node's native zstd compression is not safe on unbounded buffers: a very large
// input can fail fast inside the C module and take the whole router process
// down with STATUS_STACK_BUFFER_OVERRUN (0xC0000409) instead of throwing a
// catchable JS error (observed on big native contexts, e.g. 180K+ cached
// tokens). Compression is only an optimization, never a requirement, so a body
// above this ceiling is forwarded uncompressed rather than risking the crash.
// Tune with CODEX_ROUTER_MAX_ZSTD_COMPRESS_BYTES / MODEL_ROUTER_MAX_ZSTD_COMPRESS_BYTES.
const configuredZstdCompressBytes = Number(
  process.env.CODEX_ROUTER_MAX_ZSTD_COMPRESS_BYTES ||
    process.env.MODEL_ROUTER_MAX_ZSTD_COMPRESS_BYTES ||
    16 * 1024 * 1024,
);
const MAX_ZSTD_COMPRESS_BYTES =
  Number.isFinite(configuredZstdCompressBytes) && configuredZstdCompressBytes > 0
    ? Math.floor(configuredZstdCompressBytes)
    : 16 * 1024 * 1024;
...
async function compressedNativeBody(body, headers) {
  ...
  // The native zstd module is the crash surface here, so the guard must sit
  // before it rather than relying on a catch: fastfail exits the process, it
  // does not reject the promise, so try/catch below would never see it.
  if (body.length > MAX_ZSTD_COMPRESS_BYTES) return body;
  try {
...
async function buildRoutedRequest({ request, payload, route, agedInput, tokenMaxxing = false }) {
  ...
  // Codex's local session is the canonical conversation. A routed provider
  // receives a provider-compatible projection of the complete input on every
  // turn; it must never become a second history authority. In particular, a
  // response id issued by model A cannot be replayed after the operator picks
  // model B (or returns to native GPT). Keep the derived upstream request
  // stateless without mutating `payload`, whose input remains the pristine
  // source for every failover attempt.
  routed.store = false;
...
async function handleResponses(request, response, requestUrl) {
  ...
  try {
    ...
    if (route) {
      ...
      if (!compactV1) {
        // The caller supplied the full Codex-local transcript. Native GPT is
        // another read-only projection of that canonical history, not a
        // provider-side continuation chain. This is what makes returning from
        // any external model deterministic.
        native.store = false;
    ...
    const createResponsePipeline = (contentType) => {
      ...
      // A local GGUF can spend many minutes prefilling a large Codex tool
      // prompt across CPU and RAM before its first token. The ordinary 30s
      // empty-completion guard mistakes that expected silence for a failed
      // model and repeatedly cancels healthy work. Local Qwen instead uses an
      // SSE heartbeat below and lets llama-server finish the one real attempt.
      const localLongPrefill = route?.requestProfile === "qwen38-local";
      const guard =
        route && EMPTY_COMPLETION_RETRY && !localLongPrefill
      ...
      return { transforms, usageObserver, guard, localLongPrefill };
    };
    ...
    try {
      await pipeResponse(
        ...
        {
          leaveOpen: relayOpen,
          sseHeartbeatMs: firstPipeline.localLongPrefill ? LOCAL_SSE_HEARTBEAT_MS : 0,
        },
      );
    } catch (error) {
