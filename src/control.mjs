...
async function handleConnections(...commandArgs) {
  const script = path.join(REPO_ROOT, "src", "connections.mjs");
  const result = spawnSync(process.execPath, [script, ...commandArgs], {
    cwd: REPO_ROOT,
    env: process.env,
    input: commandArgs.includes("--with-key") || commandArgs[0] === "key-set"
  ...
  if (result.status !== 0) {
    throw new Error(String(result.stderr || "Connection operation failed.").trim());
  }
...
if (args.includes("--probe")) {
  ...
  await handleConnections(...args.slice(1));
